	...
	$.get("http://localhost:3000/categories", function(categorys) {
		$.get("http://localhost:3000/tasks", function(tasks) {
			for (var i = 0; i < tasks.length; i++) {
				...
				$("#mainbox").append("<div class='col-md-4 col-sm-6 col-xs-12'><div class='box btn-default'><div id='task-" + task.id+ "' class=''></div><h3>Titulo:  " +
					task.title + "</h3><h3r><h3>Categoria:  " + category + "</h3><h3r><h3>Tipo:  " +
					task.task_type + "</h3>" + '<button class="status btn btn-sm '+tasks_status+'" data-info=' + task.id + ' data-attr= ' + task.status + '></button></div></div>');
			};
